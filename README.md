...
- **Event cards**: moss-green paper cards branching left / right of the spine on desktop, single-column sheet layout on mobile with hyphenation, bold links. Year labels sit beside the card on desktop, above it on mobile. The currently-active role carries `event-center` instead of a side class: it sits centred astride the spine directly under the Today dot, and the walking character fades out while its box overlaps the card — reading as the figure passing behind the "current position" sign (desktop only — mobile renders it like any other card).
...
- **Events** live as `<article class="event">` elements in `index.html`. Each carries:
  ...
  - Optional class `event-center` (instead of `event-left` / `event-right`) — centres the card on the spine; used for the currently-active position. The spine builder reads its side as 0, and the card's own mark dot is hidden on desktop (the Today dot above acts as its marker).
